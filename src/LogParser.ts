...
class LogParser {
    ...
    async parseLogs(filePath: string = this.filePath) {
        if (!filePath) this.noLogFileException()
        ...
        if (!fieldsLine) this.noFieldsException()
    ...
    countAppearence(rows: string[] = this.rows, fields: string[] ) {
        if (!Array.isArray(rows)) this.rowsTypeException()
        let entryCounts = {};
        rows.forEach((row) => {
            ...
            if (entryKey) entryCounts[entryKey] ? entryCounts[entryKey]++ : entryCounts[entryKey] = 1;
        });
        this.printCounted(entryCounts);
        return entryCounts;
    }
    parseRowAndGetIndex(row: string, fields = this.fields ) {
        if (!fields) this.noFieldsException()
        ...
        if (!countedFieldIndex) this.fieldNotFoundException()
        return countedFieldIndex;
    }

    parseRowWithQuotes( row ) {
        return row.split(/"|'|`/g).reduce((values, piece) => 
            (piece.startsWith(' ') || piece.endsWith(' '))
                ? values = values.concat(piece.match(/[^ ]+/g))
                : values = values.concat(piece), []);
    }

    printCounted(entryCounts: object) {
